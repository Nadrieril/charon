...
use charon_lib::{
    ...
    logger,
    meta::InlineAttr,
    names::{ImplElem, ImplElemKind, Name, PathElem},
};
...
fn repr_name(n: &Name) -> String {
    n.name
        ...
        .map(|path_elem| match path_elem {
            ...
            PathElem::Impl(ImplElem {
                kind: ImplElemKind::Ty(..),
                ..
            }) => "<impl>",
            PathElem::Impl(ImplElem {
                kind: ImplElemKind::Trait(..),
                ..
            }) => "<trait impl>",
        })
        .join("::")
...
fn known_trait_method_call() -> Result<(), Box<dyn Error>> {
    ...
    let function = &crate_data.functions[id.index()];
    assert_eq!(
        repr_name(&function.name),
        "test_crate::<trait impl>::default"
    );
    let ItemKind::TraitItemImpl { .. } = &function.kind else {
        panic!()
    };
    Ok(())
}
