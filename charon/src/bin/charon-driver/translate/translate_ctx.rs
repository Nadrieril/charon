...
pub(crate) struct BodyTransCtx<'tcx, 'ctx, 'ctx1> {
    ...
    pub trait_clauses_map: BTreeMap<TraitDeclId, Vec<NonLocalTraitClause>>,
...
impl<'tcx, 'ctx, 'ctx1> BodyTransCtx<'tcx, 'ctx, 'ctx1> {
    ...
    pub(crate) fn get_generics(&self) -> GenericParams {
        ...
        let mut params = self.generic_params.clone();
        assert!(params.regions.is_empty());
        assert!(params
            .trait_clauses
            .iter()
            .enumerate()
            .all(|(i, c)| c.clause_id.index() == i));
        params.regions = self.region_vars[0].clone();
        params
    }
